...
#oomGraph={0: [(3, 5), {'n': 1, 's': 5, 'e': 3, 'w': 7}], 1: [(3, 6), {'s': 0, 'n': 2}], 2: [(3, 7), {'s': 1}], 3: [(4, 5), {'w': 0, 'e': 4}], 4: [(5, 5), {'w': 3}], 5: [(3, 4), {'n': 0, 's': 6}], 6: [(3, 3), {'n': 5, 'w': 11}], 7: [(2, 5), {'w': 8, 'e': 0}], 8: [(1, 5), {'e': 7}], 9: [(1, 4), {'n': 8, 's': 10}], 10: [(1, 3), {'n': 9, 'e': 11}], 11: [(2, 3), {'w': 10, 'e': 6}]}
...
while len(graph) < 500:
    # Grab the extits from the current room
    currentRoomExits = graph[player.currentRoom.id]
    unexplored = []
    # Add the exits for the current room to the graph 
    # And any rooms that have not been explored '?' add to the unexplored list
    for direction in currentRoomExits:
        if currentRoomExits[direction] == "?":
            unexplored.append(direction)

    # If there are any unexplored exits
    if len(unexplored) > 0:
        # select a random exit from the list
        random_exit = random.choice(unexplored)
        # Add that exit to the traversal path directions
        traversalPath.append(random_exit)
        # save the previous room we where in 
        previous_room_id = player.currentRoom.id
        # move the player the random direction
        player.travel(random_exit)
        exits = {}
        for exit in player.currentRoom.getExits():
            exits[exit] = '?'
        # set the room id in the graph of rooms for the room exit we randomly went into
        graph[previous_room_id][random_exit] = player.currentRoom.id
        # Add the room id for the previous room to the exits of the current room
        exits[inverse_directions[random_exit]] = previous_room_id
        # add the current rooms exits to the initial graph
        ...
        # Grab the current room and add in to the queue
